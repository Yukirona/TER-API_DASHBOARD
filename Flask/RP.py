...
def FAM_G5(wanted_data_RP):
        # - extracting and converting the data from the json
        
        nbpop = {}
        ratio = {}
        nbmen = float(wanted_data_RP['Cellule'][9]['Valeur'])
        for i in range(1,9):  
                nbpop[i] = float(wanted_data_RP['Cellule'][i+9]['Valeur'])
...
def LOG_G2(wanted_data_RP):
        # - extracting and converting the data from the json
        
        nbpop = {}
        ratio = {}
        nbmlog = float(wanted_data_RP['Cellule'][0]['Valeur'])
        for i in range(1,7):  
                nbpop[i] = float(wanted_data_RP['Cellule'][i]['Valeur'])
